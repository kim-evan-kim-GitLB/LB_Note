...
## 4. 통합 결과 (m4a 83분, 클로바 reference)

| 모드 | WER | 토큰 | 반복% | RTFx |
|---|---|---|---|---|
| 60s pipeline (rp없음) | 1.073 | 16,352 | 36.0% | 1.65 |
| 10분슬라이스 (rp없음) | 0.890 | 14,558 | 28.7% | 1.44 |
| **10분슬라이스 (rp1.2)** | **0.529** | 9,223 | **0.0%** | **2.75** |
| (클로바 ref) | — | 11,497 | — | — |

- 반복 제거 후 토큰 수가 reference(11,497)에 근접 → garbage 가 사라진 증거.
- WER 0.529 는 여전히 높은 편이나, 이는 클로바 ref 의 분절/정규화 차이 + 실세계 회의 음성의
  본질적 난이도 때문. 반복 제거 전(0.890)과 달리 이제 **측정값이 의미를 가짐**.
- 미측정: 60s pipeline + rp1.2 (코드엔 적용됨, 재실행 미수행).
