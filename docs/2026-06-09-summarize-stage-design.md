...
- **포함**: 전체 헤드라인(1~2문장) + 안건별(논의/결정/이슈) 구조화 요약, 각 항목 근거(segment+anchor).
...
- `agenda_index[]`: 안건 목록 테이블 한 줄(`no`/`title`/한 줄 `summary`). 상세 `agenda[]` 와 `no` 로 매핑.
...
> 요약 off(아래 §6) 시 `summary` 는 빈 구조체 `{"headline": "", "agenda": []}` 로 둔다(타입 일관).
...
3. **seg_id 유효성** — 인용 id 가 실제 입력 segment 집합에 존재해야 함(환각 인용 차단). 존재하지
   않는 id 는 제거, 결과로 evidence 가 비면 항목 드롭.
4. **길이 상한** — `headline` ≤ 2문장. 섹션 항목 수는 회의 규모 비례(예: 안건당 섹션별 ≤ N,
   잠정값은 1차 측정 후 확정). 과도 분해 금지(한 결정을 여러 항목으로 쪼개지 말 것).
5. 최종 실패 → 해당 부분만 빈 배열 + run 관측성에 flag 기록(전체 멈춤 없음).
...
```
src/postprocess/
  summarize_schema.py      # SummaryItem/AgendaBlock/MeetingSummary dataclass + 버전 필드
  stages/summarize.py      # SummarizeStage(회의 1콜, transcript_with_ids 주입, 파싱→스키마)
  validate.py              # 요약 게이트[D'] 추가(근거존재·seg_id유효·길이)
prompts/summarize.ko.md    # 요약 프롬프트(버전 명시, 인젝션 격리 — 추출과 동일 규약)
src/postprocess/web_contract.py  # summary 직렬화 변경(str → 구조체), 회의록.md 렌더
src/web/{app,service}.py   # WEB_SUMMARIZE_BACKEND 게이팅, process 경로 summary 연결
eval/gold_summary.json     # 결정·이슈 정답셋(회귀 고정)
tests/test_summarize_*.py  # 스키마·근거·anchor 결정성·회귀 회수율
```
