...
| 60s pipeline (rp1.2) | 0.652 | 10333 | 3.0% | 2.23 |
| **10분슬라이스 (rp1.2)** | **0.529** | **9223** | **0.0%** | **2.75** |

→ rp1.2 로 반복 완전 제거 + WER 40%↓ + 속도 2배. **같은 rp1.2 조건에서도 10분슬라이스 > 60s** (WER 0.529 vs 0.652, 반복 0% vs 3%) = 청크 경계 적을수록 좋다는 구조적 이점 확증. 회귀 없음 → **feat/vad base(`677c4ff`) 확정**. (rp 값: 1.3은 1.2 대비 반복 개선 없이 distortion만↑ → 1.2 확정)
