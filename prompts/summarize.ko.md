...
<!-- prompt_version: summarize-ko-1.1 -->
...
### 오인식·환각 방어 (v1.1, 중요)
transcript는 음성인식(STT) 결과이며 **없는 발화가 지어내진 구간이 섞일 수 있다.** 다음은
회의 발화가 아니므로 요약하지 말고 인용하지도 마라:
- **회의 맥락과 무관한 외국어 문장**(특히 영어 한 덩어리). 한국어 회의 중 갑자기 문맥이 끊긴
  영어 문장이 나오면 무음·잡음 구간의 환각이다.
- 같은 문구가 의미 없이 반복되는 구간(반복 붕괴).
- 영상 자막성 상용구("thank you for watching", "subtitles by ..." 등).
단, **한국어 발화에 영어 용어가 섞인 것은 정상이다**(예: "Whisper 모델이 Tiny, Base…",
"action item list를 도출해"). 이건 그대로 요약 대상이다 — 영어 단어가 보인다는 이유로 버리지 마라.

### 저신뢰 표시 줄 (`[12]~`)
segment_id 뒤에 `~` 가 붙은 줄은 **인식 신뢰도가 낮다**(비한국어 우세 등). 참고는 해도 되지만
**그 줄만을 근거로 하는 항목은 만들지 마라.** 신뢰 줄(`~` 없는 줄) 근거가 함께 있을 때만 채택한다.

### 출력 언어
모든 출력 텍스트(`title`, `summary`, `text`, `subject`)는 **한국어**로 쓴다. transcript에 외국어가
섞여 있어도 요약문은 한국어다. 고유명사·기술용어는 원문 표기를 유지한다.
