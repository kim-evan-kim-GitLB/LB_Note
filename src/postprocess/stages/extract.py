...
def build_messages(
    ...
    low_conf_ids(언어 게이트): 저신뢰 segment 는 `[id]~` 로 표시해 단독 근거 사용을 막는다.
    """
    body = transcript_with_ids(segments, low_conf_ids)
...
class ExtractStage:
    ...
    def run(
        ...
        messages = build_messages(
            ...
            system_tmpl,
            user_tmpl,
            summary_hints=ctx.get("summary_hints"),
            low_conf_ids=ctx.get("low_conf_ids"),
        )
